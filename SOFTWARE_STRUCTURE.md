...
- `.github/workflows/release.yml`: workflow GitHub Actions chạy khi push tag `v*`, build Windows bằng `npm run build` + `electron-builder --publish never`, rồi upload installer/portable `.exe` lên GitHub Release tương ứng.
